...
export default function McpTokenManager() {
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <div>
        <h3 className="font-semibold">Conectar con Claude / Claude Cowork</h3>
      ...
      <div className="rounded-lg bg-gray-50 p-3 text-sm">
        <p className="font-medium">URL del servidor MCP</p>
      ...
      <form onSubmit={handleCreate} className="flex items-end gap-2">
        <div className="flex-1 space-y-1">
          <label className="text-sm font-medium text-gray-700">Nombre del token</label>
          <input
            value={name}
      ...
      {newToken && (
        <div className="space-y-1 rounded-lg border border-amber-300 bg-amber-50 p-3">
          ...
          <code className="block break-all rounded bg-white px-2 py-1 text-xs">{newToken}</code>
        </div>
      )}
      ...
      <ul className="divide-y divide-gray-100">
        ...
        {tokens.length === 0 && (
          <p className="py-2 text-sm text-gray-400">Todavía no generaste ningún token.</p>
        )}
      </ul>
    </div>
  );
